...
# Features to compare (exclude mpg itself)
vars <- setdiff(names(Auto), c("mpg", "mpg01","name"))

# Boxplots
par(mfrow=c(2,4))
for (v in vars) {
  boxplot(Auto[[v]] ~ Auto$mpg01, main=v, xlab="mpg01", ylab=v)
}

# Scatterplots
par(mfrow=c(2,4))  
for (v in vars) {
  plot(Auto$mpg01, Auto[[v]], main=v, xlab="mpg01", ylab=v)
}
...
lda.pred <- predict(lda.fit, Auto[-train, ])$class
mean(lda.pred != Auto$mpg01[-train])

#e)
qda.fit <- qda(mpg01 ~ cylinders+displacement+weight, data=Auto, subset=train)
qda.pred <- predict(qda.fit, Auto[-train, ])$class
mean(qda.pred != Auto$mpg01[-train])

#f)
glm.fit <- glm(mpg01 ~ cylinders+displacement+weight, data=Auto, subset=train, family=binomial)
glm.probs <- predict(glm.fit, Auto[-train, ], type="response")
glm.pred <- ifelse(glm.probs > 0.5, 1, 0)
mean(glm.pred != Auto$mpg01[-train])
